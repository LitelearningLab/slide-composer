...
export async function convertToWav(audioBuffer: Buffer): Promise<Buffer> {
  ...
  try {
    ...
    if (process.platform === "win32") {
      const wingetPath = `${process.env.LocalAppData}\\Microsoft\\WinGet\\Links\\ffmpeg.exe`;
...
/**
 * Text-to-Speech: Converts text to speech using OpenAI's TTS API.
...
export async function textToSpeech(
  ...
  console.log(`[TTS] Starting text-to-speech conversion. Voice: ${voice}, Text length: ${text.length}`);
  ...
  try {
    ...
    console.log(`[TTS] Response received from OpenAI`);
    const arrayBuffer = await response.arrayBuffer();
    const buffer = Buffer.from(arrayBuffer);
    console.log(`[TTS] Successfully converted text to speech. Audio size: ${buffer.length} bytes`);
