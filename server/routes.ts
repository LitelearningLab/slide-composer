...
export async function registerRoutes(
  ...
  app.post("/api/presentations/generate", async (req, res) => {
    try {
      ...
      const { title: requestTitle, rawData, theme } = parseResult.data;

      const { slides } = parseContentToSlides(rawData, requestTitle);
...
Respond with JSON:
{
  ...
  // Register audio/voice chat routes
  ...
  app.get("/api/proxy-html", async (req, res) => {
    try {
      ...
      if (!response.ok) {
        return res.status(response.status).json({ error: "Failed to fetch remote HTML" });
      }
